...
static int irda_fw_update(struct ir_remocon_data *ir_data)
{
	...
	int i=0, k, ret, ret2, checksum, checksum2, frame_count = 0;
	u8 buf_ir_test[8];
	const u8 *IRDA_fw;
	ret = 0;
...
//			goto err_bootmode;
	...
	return 0;
err_update:
	if((update_retry_count == MAX_UPDATE_RETRY - 1) && ((buf_ir_test[2] << 8 | buf_ir_test[3]) == 0x101)) {
		download_pass = 1;
		update_retry_count = 0;
		printk(KERN_ERR "%s: using old firmware\n", __func__);
		goto fw_update_end;
	}
